...
Human review is required by default at every `moe work` turn and before each `moe sign` gate. A future **yolo mode** can collapse those into one operator command — `moe yolo <proj> <req> --through <stage>` — which walks the document graph autonomously, drafting every document the request needs, then crossing each stage gate up to `<stage>` without pausing for input. Default upper bound is `pr`; `deploy` is never crossable by yolo (hard-coded, not a flag), so the worst-case outcome is a PR sitting open for human review, not a shipped change. Yolo is the right-hand end of a spectrum — operator watches the shell, Ctrl-C is always the off switch, every step still lands as trailer-tagged commits on main so `moe unsign` and `git revert` unwind cleanly.
...
| Yolo mode (`moe yolo <req> --through <stage>` — autonomously drafts every document and crosses every gate up to `<stage>`, capped at `pr`; distinct from Claude Code's in-session `--dangerously-skip-permissions`, though yolo generally wants both) | Deferred | Every request passes human review at every turn and gate in v1. |
