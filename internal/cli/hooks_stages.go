...
var openHooksStage func(stage, projectID, runID string, suppressNextStage bool, stdout, stderr io.Writer) int

func init() {
	openHooksStage = func(stage, projectID, runID string, suppressNextStage bool, stdout, stderr io.Writer) int {
		switch stage {
		case hooksCodeDoc:
			return openHooksCode(projectID, runID, true, suppressNextStage, stdout, stderr)
		default:
			moePrintf(stderr, "hooks: openHooksStage: unknown stage %q\n", stage)
			return 1
		}
	}
	registerHeadlessDispatcher(hooksWorkflow, func(stage, projectID, runID string, suppressNextStage bool, stdout, stderr io.Writer) int {
		return openHooksStage(stage, projectID, runID, suppressNextStage, stdout, stderr)
	})
}
