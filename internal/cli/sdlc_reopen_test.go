...
// only seeds a "design" doc. A non-sdlc prior would either error at
...
func TestSDLCReopenRefusesNonSDLC(t *testing.T) {
	root := seedCloseFixture(t, "tele", "chat-prior", "chat", run.StatusClosed)
	t.Setenv("MOE_HOME", root)
	t.Setenv("NO_COLOR", "1")

	var out, errb bytes.Buffer
	code := Run([]string{"sdlc", "reopen", "tele/chat-prior"}, &out, &errb)
	if code == 0 {
		t.Fatalf("expected non-zero, stdout=%q", out.String())
	}
...
func TestDashClosedNonSDLCHasNoReopenMarker(t *testing.T) {
	...
	trailerstest.SeedRun(t, root, "tele", "chat-dead", "chat", run.StatusClosed)
	trailerstest.CommitTrailer(t, root, "Close chat run tele/chat-dead",
		"MoE-Run: chat-dead\nMoE-Project: tele\nMoE-Workflow: chat",
	...
	if !strings.Contains(out.String(), "chat:closed") {
		t.Fatalf("expected chat closed row, got:\n%s", out.String())
	}
