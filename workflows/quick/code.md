...
is to produce the smallest diff that resolves it. `moe workflow quick push` is
...
- **Match the brief.** The canvas is the contract. If implementation
  forces a deviation, surface it in the PR body — don't smuggle it in.
- **Be the smallest change that solves the problem.** Adjacent cleanups,
  ...
  follow-up if they're worth doing. If the work starts feeling bigger
  than a quick fix, stop and tell the operator — this run probably
  wants the full sdlc workflow instead.
...
- **Speculative scope.** Extra knobs, new abstractions, or "future-proofing"
  the brief didn't ask for. The PR is the brief rendered in code, not a
  second design pass.
- **Silent behavior changes.** If you touched something the brief didn't
...
- **Hold the line on scope.** If the operator asks for a scope expansion
  ...
  is how PRs become unreviewable — and quick is for small changes.
...
edits don't ship until you commit them in this clone.** `moe workflow quick push`
...
1. The diff resolves the brief with no unexplained extras.
...
5. The operator has what they need to run `moe workflow quick push` — or to say
   "not yet, because X."
