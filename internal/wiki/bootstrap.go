...
// EnsureManagedDocs is the engine's bootstrap step. For each
// cfg.ManagedDocs entry not yet on disk it creates the file with a
// "# <Title>\n\n" stub. Runs at session open so the schema invariants
// are satisfied for the rest of the turn — the agent's edits during the
// session land on top of the stub, and the per-turn commit ships the
// freshly-stubbed file alongside the agent's content.
//
// Returns true when at least one stub was created (the "fresh wiki"
// signal callers can use to adjust kickoff framing).
func EnsureManagedDocs(cfg Config) (bool, error) {
	if len(cfg.ManagedDocs) == 0 {
		return false, fmt.Errorf("wiki: engine requires ManagedDocs to be non-empty")
	}
