...
func (s *Server) registerRoutes() {
	...
	// The operator input channel: what runs have asked and what the
	// operator has pushed at them. Read-only queue plus one write POST,
	// both journal-only — the write lands a commit and starts nothing,
	// same as the advance mark.
	s.router.HandleFunc("GET /input", s.handleInputQueue)
	s.router.HandleFunc("POST /run/{project}/{slug}/input", s.handleInput)
...
// syncWriter is the io.Writer handed to callers that write to it
// unconditionally — the heartbeat gate's Due and usage.Gather's parse
// warnings. Same sink as logf; never nil for that reason.
