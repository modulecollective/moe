...
func runNew(workflowName string, args []string, stdout, stderr io.Writer) int {
	...
	//
	// --ship / --chain are the same consent ladder the stage verbs
	// carry, mapping to `!!` / `!!!`. Opening a run and riding it is one
	// intent an operator has in one breath — "promote this idea and ride
	// the chain" — and the flag path used to stop at `!!`, so that intent
	// wasn't sayable in one command even though the interactive open
	// prompt has always offered the whole ladder.
	//
	...
	ship := fs.Bool("ship", false, "open the run and cascade every stage headless to the ship (the flag twin of `!!` at the chain prompt; mutually exclusive with --park/--chain)")
	chain := fs.Bool("chain", false, "open the run, ship it, and ride the chain it heads (the flag twin of `!!!`; mutually exclusive with --park/--ship)")
	fs.Usage = func() {
		moePrintf(stderr, "usage: moe %s new [--workspace <name>] [--agent <name>] [--seed] [--park|--ship|--chain] <project>/<slug>\n", workflowName)
		moePrintf(stderr, "       moe %s new [--workspace <name>] [--agent <name>] [--park|--ship|--chain] --from-idea <project>/<slug>\n", workflowName)
		fs.PrintDefaults()
	}
	...
	if !ok {
		moePrintf(stderr, "%s new: --ship and --chain are one ladder — pick one\n", workflowName)
		return 2
	}
	if *seed && *fromIdea != "" {
