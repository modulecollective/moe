...
You are running headless: the chain prompt's `o` keystroke, the
`!<stage>` / `!!` cascade driver, or the queue walker invoked this
stage. There is no operator on stdin and you only get one turn —
the runner exits as soon as your turn ends.
...
requested change is too large for one headless turn — refuse by exiting
without writing to the canvas file. The runner asserts canvas existence
at commit time, so silent refusal is enough to stop the chain. The
