...
func TestEmbeddedFragmentsCoverRegisteredStages(t *testing.T) {
	// `push` is operational (no stage session) and is the last stage
	// with no fragment: idea and intent both open one via
	// `edit --chat`, so their fragments are shipped and covered here.
...
func TestBuildSystemPromptInjectsSdlcDesignFragment(t *testing.T) {
	...
	got, err := buildSystemPrompt(root, md, "design", "", false, nil)
	if err != nil {
		t.Fatal(err)
	}
...
func TestBuildSystemPromptMissingFragmentIsNotAnError(t *testing.T) {
	...
	got, err := buildSystemPrompt(root, md, "ghost", "", false, nil)
	if err != nil {
...
func TestBuildSystemPromptSectionsEndWithNewline(t *testing.T) {
	...
	md := &run.Metadata{ID: "fix-it", Project: "tele", Workflow: "sdlc"}
	got, err := buildSystemPrompt(root, md, "code", "", false, wikiCfg)
	if err != nil {
		t.Fatal(err)
	}
