...
// know which sweep finished and whether it succeeded; the activity
// log on the per-run page carries the detail.
type notifyPayload struct {
	// ID is always "heartbeat:<project>". The heartbeat is serve's only
	// spawner, so every child serve reaps is a sweep and the id already
	// names the project that was swept.
	ID     string `json:"id"`
