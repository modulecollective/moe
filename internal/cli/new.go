...
func runNew(workflowName string, args []string, stdout, stderr io.Writer) int {
	...
	agentOverride := fs.String("agent", "", "agent backend for this run (claude/codex). Explicit values persist to run.json; omitted values resolve at stage time via $MOE_AGENT, then 'moe config get default_agent', then claude")
	fs.Usage = func() {
