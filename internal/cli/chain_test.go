...
func TestParseChainEditFileExtractsFirstTokenSkipsCommentsAndBlanks(t *testing.T) {
	body := `# moe chain edit
#
# instructions
projA/run-foo          # orphan

projA/run-bar  # chained-from projA/run-foo
...
`
	...
	want := []string{"projA/run-foo", "projA/run-bar", "projB/run-baz"}
...
func TestParseChainEditFileRejectsDuplicate(t *testing.T) {
	body := "p/foo\np/foo\n"
	_, err := parseChainEditFile(body)
	if err == nil {
...
func TestParseChainEditFileEmpty(t *testing.T) {
	...
	if len(got) != 0 {
		t.Errorf("comment-only file: want empty list, got %v", got)
	}
}
...
func TestDiffChainEditNewChainOnAllOrphans(t *testing.T) {
	...
	desired := []string{"p/a", "p/b", "p/c"}
	live := map[string]string{}
	adds, removes := diffChainEdit(desired, live)
