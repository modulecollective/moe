...
func TestIdeaEditRefusesNonIdeaRun(t *testing.T) {
	// Guard that `moe idea edit` doesn't operate on an sdlc/kb/quick
...
// Migration: pre-existing projects/<p>/ideas/*.md get converted to
// idea runs in a single commit, with the source files removed in the
// same commit. Collisions against existing runs auto-suffix.
func TestIdeaMigrateConvertsLegacyIdeas(t *testing.T) {
	root := newTestBureaucracy(t)
	markBureaucracy(t, root)
	seedProject(t, root, "tele")
	t.Setenv("MOE_HOME", root)
	...
	// Drop two legacy idea files and commit them as-is.
	ideasDir := filepath.Join(root, "projects", "tele", "ideas")
	...
	for slug, body := range map[string]string{
		...
		if err := os.WriteFile(filepath.Join(ideasDir, slug+".md"), []byte(body), 0o644); err != nil {
			t.Fatal(err)
		}
	}
	addCmd := exec.Command("git", "-C", root, "add", "-A")
	...
	commitCmd := exec.Command("git", "-C", root, "commit", "-m", "legacy ideas")
	if out, err := commitCmd.CombinedOutput(); err != nil {
		t.Fatalf("git commit: %v\n%s", err, out)
	}

	var out, errb bytes.Buffer
	code := Run([]string{"idea", "migrate"}, &out, &errb)
	if code != 0 {
		t.Fatalf("exit=%d stderr=%q", code, errb.String())
	}

	// Both legacy files are gone.
	for _, slug := range []string{"first-one", "second-one"} {
		if _, err := os.Stat(filepath.Join(ideasDir, slug+".md")); !os.IsNotExist(err) {
			t.Fatalf("legacy idea file %s should be gone, stat err=%v", slug, err)
		}
	}
	// Both runs exist, each with run.json + canvas.
	for slug, wantTitle := range map[string]string{
		"first-one":  "First one",
		"second-one": "Second one",
	} {
		mdBody, err := os.ReadFile(filepath.Join(root, "projects", "tele", "runs", slug, "run.json"))
		if err != nil {
			t.Fatalf("run.json missing for %s: %v", slug, err)
		}
		if !strings.Contains(string(mdBody), `"workflow": "idea"`) {
			t.Fatalf("%s run.json not idea workflow:\n%s", slug, mdBody)
		}
		if !strings.Contains(string(mdBody), `"title": "`+wantTitle+`"`) {
			t.Fatalf("%s title mismatch:\n%s", slug, mdBody)
		}
		canvas, err := os.ReadFile(ideaCanvas(root, "tele", slug))
		if err != nil {
			t.Fatalf("canvas missing for %s: %v", slug, err)
		}
		if !strings.Contains(string(canvas), "# "+wantTitle) {
			t.Fatalf("%s canvas missing title:\n%s", slug, canvas)
		}
	}

	// One commit lands all of it. --no-renames keeps git from
	// collapsing the A+D of identical content into an R (rename) so we
	// can assert on the operations independently.
	head := gitLog(t, root, "-1", "--name-status", "--no-renames", "--format=%s%n%b")
	if !strings.Contains(head, "migrate: convert 2 idea file(s) to idea runs") {
		t.Fatalf("commit subject wrong:\n%s", head)
	}
	for _, want := range []string{
		"D\tprojects/tele/ideas/first-one.md",
		"D\tprojects/tele/ideas/second-one.md",
		"A\tprojects/tele/runs/first-one/run.json",
		"A\tprojects/tele/runs/second-one/run.json",
		"A\tprojects/tele/runs/first-one/documents/idea/content.md",
		"A\tprojects/tele/runs/second-one/documents/idea/content.md",
	} {
		if !strings.Contains(head, want) {
...
func TestIdeaMigrateNoLegacyFilesIsNoOp(t *testing.T) {
	root := newTestBureaucracy(t)
	markBureaucracy(t, root)
	seedProject(t, root, "tele")
	t.Setenv("MOE_HOME", root)
	...
	afterHead := gitLog(t, root, "-1", "--format=%H")
	if beforeHead != afterHead {
		t.Fatalf("no-op migrate created a commit:\nbefore=%s\nafter=%s", beforeHead, afterHead)
	}
}
