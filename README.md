...
Human review is required by default before `moe sign pr`. A future **auto-sign mode** (MoE-internal "yolo" — distinct from Claude Code's `--dangerously-skip-permissions`, which operates at the tool-call layer inside a single session) can auto-sign for well-understood request types where the agents consistently produce good output.
...
| Auto-sign mode ("yolo" — auto-`moe sign pr`, not to be confused with Claude Code's tool-level `--dangerously-skip-permissions`) | Deferred | Every request passes a human review gate in v1. |
