...
var openMetaMoeStage func(stage, projectID, runID string, suppressNextStage bool, stdout, stderr io.Writer) int

func init() {
	openMetaMoeStage = func(stage, projectID, runID string, suppressNextStage bool, stdout, stderr io.Writer) int {
		switch stage {
		case metaMoeReportDoc:
			return openMetaMoeReport(projectID, runID, true, suppressNextStage, stdout, stderr)
		default:
			moePrintf(stderr, "meta-moe: openMetaMoeStage: unknown stage %q\n", stage)
			return 1
		}
	}
	registerHeadlessDispatcher(metaMoeWorkflow, func(stage, projectID, runID string, suppressNextStage bool, stdout, stderr io.Writer) int {
		return openMetaMoeStage(stage, projectID, runID, suppressNextStage, stdout, stderr)
	})
}
