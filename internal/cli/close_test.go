...
func TestSDLCCloseRemovesSandboxAndCommits(t *testing.T) {
	...
	code := Run([]string{"sdlc", "close", "tele", "abandon-me"}, &out, &errb)
	if code != 0 {
...
func TestSDLCCloseWithoutSandboxIsFine(t *testing.T) {
	...
	var out, errb bytes.Buffer
	code := Run([]string{"sdlc", "close", "tele", "never-opened"}, &out, &errb)
	if code != 0 {
		t.Fatalf("exit=%d stderr=%q", code, errb.String())
	}
...
func TestSDLCCloseRefusesPushed(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			var out, errb bytes.Buffer
			code := Run([]string{"sdlc", "close", "tele", "done-" + tc.name}, &out, &errb)
			if code == 0 {
...
func TestSDLCCloseWorkflowMismatch(t *testing.T) {
	...
	var out, errb bytes.Buffer
	code := Run([]string{"sdlc", "close", "tele", "not-sdlc"}, &out, &errb)
	if code == 0 {
...
func TestSDLCCloseMissingRun(t *testing.T) {
	...
	var out, errb bytes.Buffer
	code := Run([]string{"sdlc", "close", "tele", "ghost"}, &out, &errb)
	if code == 0 {
...
func TestKBCloseBumpsStatusAndCommits(t *testing.T) {
	...
	var out, errb bytes.Buffer
	code := Run([]string{"kb", "close", "tele", "dead-end"}, &out, &errb)
	if code != 0 {
		t.Fatalf("exit=%d stderr=%q", code, errb.String())
	}
...
func TestKBCloseLeavesUnrelatedPathsAlone(t *testing.T) {
	...
	var out, errb bytes.Buffer
	code := Run([]string{"kb", "close", "tele", "kb-run"}, &out, &errb)
	if code != 0 {
		t.Fatalf("exit=%d stderr=%q", code, errb.String())
	}
...
func TestKBCloseRefusesTerminal(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			beforeHead := gitLog(t, root, "-1", "--format=%H")
			var out, errb bytes.Buffer
			code := Run([]string{"kb", "close", "tele", "kb-done-" + tc.name}, &out, &errb)
			if code == 0 {
				t.Fatalf("expected non-zero on %s, stdout=%q", tc.status, out.String())
			}
			if !strings.Contains(errb.String(), "already") {
...
func TestSDLCCloseRegisteredInUsage(t *testing.T) {
	...
	if !ok {
		t.Fatal("sdlc command missing from registry")
	}
	var out, errb bytes.Buffer
...
func TestKBCloseRegisteredInUsage(t *testing.T) {
	cmd, ok := commands["kb"]
	if !ok {
		t.Fatal("kb command missing from registry")
	}
	var out, errb bytes.Buffer
	if code := cmd.Run(nil, &out, &errb); code != 0 {
		t.Fatalf("exit=%d stderr=%q", code, errb.String())
